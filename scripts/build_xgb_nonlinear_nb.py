...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "differ **only** in whether the cognitive parameters are available. Gradient-boosted trees "
    "can capture nonlinear interactions among the 30 parameters that CCA/rCCA cannot, so no "
    "detected gain here is evidence against a nonlinear cognition→symptom signal *recoverable by "
    "this XGBoost pipeline on this sample* (one fixed depth-3 configuration, ~415 fitting rows "
    "per SHARP fold — not a model-agnostic proof of absence).\n\n"
    "Inference uses the paper-faithful **SHARP null-constrained score test** (Zeng et al. 2026, "
    "S7.6/S7.8) on the paired per-half difference in held-out R² (full − null): a valid, "
    "fold-dependence-aware, one-sided (full > null) p with a test-inversion CI. Both arms use the "
    "identical algorithm (fixed shallow trees; rounds tuned by early stopping inside each training "
    "fold). The raw half-statistics are retained in the artifact.\n\n"
...
cells.append(nbf.v4.new_markdown_cell(
    "## Conclusion\n\n"
    "**No incremental predictive gain detected.** For (nearly) every survey score the held-out "
    "R² gain from adding the cognitive parameters is ≤ 0 — the full model does not out-predict "
    "the age+sex baseline — and **no target survives BH-FDR** under the one-sided SHARP score "
    "test. The closest-to-zero targets (`hitop_hypsom`, `hitop_welbe`) are still non-significant.\n\n"
    "The justified claim is deliberately narrow: *no incremental predictive gain for these "
    "features (the 30 fitted parameters) with this XGBoost pipeline (fixed depth-3, ~415 fitting "
    "rows per SHARP fold) on this training-half sample* — **not** a model-agnostic proof that no "
    "nonlinear cognition→symptom relationship exists. Within that scope it corroborates the "
    "CCA-ladder and Anna Karenina nulls.\n\n"
    "Two robustness checks agree: the full-sample K-fold cross-check (§3, ~830 rows/fold, no "
    "split-half) only shrinks the gaps toward zero (a few tip marginally positive, none "
    "distinguishable from noise), and swapping the 30 parameters for the 4 per-task summary "
    "scores (§4) gives the same null. The result holds across these feature representations and "
    "evaluation schemes.\n\n"
